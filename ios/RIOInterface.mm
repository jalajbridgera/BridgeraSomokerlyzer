...
@synthesize totalCount,recorderCount;
...
- (void)startListening:(CommunicateDeviceViewController*)aListener {
    totalCount = 0;
    recorderCount = 0;
                            ...
                            AudioBufferList                *ioData)
{
    ...
    if (read > inNumberFrames) {
        ...
        [THIS->listener frequencyChangedWithValue:(float)bin*(THIS->sampleRate/bufferCapacity)];
        // printf("Dominant frequency: %f   bin: %d \n", bin*(THIS->sampleRate/bufferCapacity), bin);
        /*
         if(bin*(THIS->sampleRate/bufferCapacity) >=1000)
         {
         ...
         //            [THIS->listener frequencyChangedWithValue:bin*(0.0)];
         //            printf("Dominant frequency: %f   bin: %d \n", bin*(0.0), bin);
         }
         */
    }
...
- (void)realFFTSetup {
    ...
    NSLog(@"mf %ul - sint16 %lu - sizeofloat %lu", maxFrames, sizeof(SInt16),sizeof(float));
}
...
- (void)initializeAudioSession {
    ...
    /*
     ...
     if (!success)  NSLog(@"AVAudioSession error setting category:%@",err);
     ...
     NSLog(@"property size:%@",[NSNumber numberWithUnsignedInt:propertySize]);
     
     AudioSessionGetProperty(kAudioSessionProperty_AudioRoute, &propertySize, &audioInputIsAvailable);
     
     NSLog(@"rio %@",audioInputIsAvailable);
     //
     */
    //activate the audio session
    success = [session setActive:YES error:&err];
    if (!success) NSLog(@"AVAudioSession error activating: %@",err);
    ...
    //    {
    //        AVAudioSessionPortDescription *aspd = [availableInputs objectAtIndex:i];
    //        NSLog(@"input %@", [aspd portName]);
    //    }
    //
    //
...
- (void)createAUProcessingGraph {
    ...
    //found this http://stackoverflow.com/questions/3767527/iphone-audiobufferlist-init-and-release
    bufferList= static_cast<AudioBufferList *>(calloc(1, offsetof(AudioBufferList, mBuffers) + (sizeof(AudioBuffer) * 1)));//1 is numchannels
    ...
    NSLog(@"%lu - %lu",sizeof(AudioBuffer),bytesPerSample );
}
