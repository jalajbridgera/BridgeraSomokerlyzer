...
static CommunicateDeviceViewController *sharedInstance = nil;
//static dispatch_once_t onceToken;
//dispatch_once(&onceToken, ^{
   
sharedInstance = [[self alloc] init:devicePin];
//});
//return sharedInstance;
}


- (id)init:(int)devicePin {
        if (self = [super init]) {
// write your code here
            
            [self loadInitialValues:devicePin];
        }
    return self;
 }
...
- (void)loadInitialValues:(int)devicePin
{
//    RIOInterface *new_rioRef = [RIOInterface sharedInstance];
//    [new_rioRef setSampleRate:44100];
//    [new_rioRef setFrequency:294];
//    [new_rioRef initializeAudioSession];
//
    arr_Values = [[NSArray alloc] initWithObjects: @2,@3,@1,@4,@5,@6,@8,@14,@15,@20,@25, nil];
    NSLog(@"%@",[arr_Values objectAtIndex: arc4random()% arr_Values.count]);
...
//    [super viewWillAppear: animated];
   // [audioRouteLabel setText: TranslatedString([self audioRouteAvailable], nil)];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(startTimerForTest:)
                                                 name:@"startTest"
                                               object:nil];
}

-(void)startTimerForTest:(NSNotification*)notification
{
    [self startCountDown];
}
...
//            else{
            ...
            /*
             {"data":{"buttonName":"","isButtonClickable":false,"isButtonInvisible":false,"isDeviceMode":true,"ppmValue":0,"time":15,"viewType":0},"key":"timeUpdate"}
             */
                dispatch_async(dispatch_get_main_queue(), ^{
                    // UI update code here.
                    [self.startListenBtn setHidden:NO];
                    countdown = 19;
                    self.beginTimer = [NSTimer scheduledTimerWithTimeInterval:1.0   target:self   selector:@selector(beginTimerFired)  userInfo:nil  repeats:YES];
                    ...
                    [UIView animateWithDuration:1.0f animations:^{
                      ...
                      //  self.audioRouteLabel.hidden = YES;
...
-(void)disabledDeviceTimerFired
{
    ...
    if(connected.location != NSNotFound || overrideConnectedDevice ==YES ) {
        ...
        // UPDATE BUTTON STATE CALLBACK //
        [self callbackUpdateBeginTestButtonState:true];
    }else{
         [self callbackUpdateBeginTestButtonState:false];
    }
    
}

-(void)callbackUpdateBeginTestButtonState:(BOOL)isButtonClickable{
    
    dict_Data_To_Send = [[NSMutableDictionary alloc] init];
    [dict_Data_To_Send setValue:@"buttonUpdate" forKey:@"key"];
    NSMutableDictionary *dict_Buffer_Data = [[NSMutableDictionary alloc] init];
    [dict_Buffer_Data setValue:@(isButtonClickable) forKey:@"isButtonClickable"];
    [dict_Buffer_Data setValue:@NO forKey:@"isButtonInvisible"];
    [dict_Buffer_Data setValue:@YES forKey:@"isDeviceMode"];
    [dict_Buffer_Data setValue:[NSNumber numberWithInt:0] forKey:@"ppmValue"];
    [dict_Buffer_Data setValue:[NSNumber numberWithInt:0] forKey:@"time"];
    [dict_Buffer_Data setValue:[NSNumber numberWithInt:0] forKey:@"viewType"];
    [dict_Data_To_Send setValue:dict_Buffer_Data forKey:@"data"];
    
    NSError *error;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:dict_Data_To_Send
    ...
    if (! jsonData) {
        ...
        NSLog(@"%@",str_JSON);
        NSDictionary *dict_UserINfo = [[NSDictionary alloc] initWithObjectsAndKeys:str_JSON,@"info", nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"buttonUpdate" object:self userInfo:dict_UserINfo];
    }
    
}

-(void)updateCallBackWithDeviceMode:(BOOL)deviceModeToSend
{
    deviceMode = deviceModeToSend;
    dict_Data_To_Send = [[NSMutableDictionary alloc] init];
    [dict_Data_To_Send setValue:@"deviceMode" forKey:@"key"];
    NSMutableDictionary *dict_Buffer_Data = [[NSMutableDictionary alloc] init];
    if (self.continueBtn.isUserInteractionEnabled) {
        [dict_Buffer_Data setValue:@YES forKey:@"isButtonClickable"];
    }else{
        [dict_Buffer_Data setValue:@NO forKey:@"isButtonClickable"];
    }
    [dict_Buffer_Data setValue:@(self.continueBtn.isHidden) forKey:@"isButtonInvisible"];
    [dict_Buffer_Data setValue:@(deviceMode) forKey:@"isDeviceMode"];
    [dict_Buffer_Data setValue:[NSNumber numberWithInt:0] forKey:@"ppmValue"];
    [dict_Buffer_Data setValue:[NSNumber numberWithInt:0] forKey:@"time"];
    [dict_Buffer_Data setValue:[NSNumber numberWithInt:0] forKey:@"viewType"];
    [dict_Data_To_Send setValue:dict_Buffer_Data forKey:@"data"];
    ...
    if (! jsonData) {
        ...
        NSLog(@"%@",str_JSON);
        NSDictionary *dict_UserINfo = [[NSDictionary alloc] initWithObjectsAndKeys:str_JSON,@"info", nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"deviceMode" object:self userInfo:dict_UserINfo];
    }
}

-(void)beginTimerFired
{
...
//        enableContinueTimer =[NSTimer scheduledTimerWithTimeInterval:24.0   target:self   selector:@selector(enableContinueBtn:)  userInfo:nil  repeats:NO];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(24 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                //Send Result Data To Call Back
              int ppm = round([self findPeakPPM] *100)/100;
            
            NSMutableDictionary* dictTimeUpdate = [[NSMutableDictionary alloc] init];
            [dictTimeUpdate setValue:@"result" forKey:@"key"];
            NSMutableDictionary* dict_Buffer = [[NSMutableDictionary alloc] init];
          
            [dict_Buffer setValue:@NO forKey:@"isButtonClickable"];
            [dict_Buffer setValue:@NO forKey:@"isButtonInvisible"];
            [dict_Buffer setValue:@NO forKey:@"isDeviceMode"];
            [dict_Buffer setValue:[NSNumber numberWithInt:ppm] forKey:@"ppmValue"]; 
            [dict_Buffer setValue:[NSNumber numberWithInt:0] forKey:@"time"];
            [dict_Buffer setValue:[NSNumber numberWithInt:0] forKey:@"viewType"];
            [dictTimeUpdate setValue:dict_Buffer forKey:@"data"];
            
            NSError *error;
            NSData *jsonData = [NSJSONSerialization dataWithJSONObject:dictTimeUpdate
                                                               options:(NSJSONWritingOptions) (NSJSONWritingPrettyPrinted)
                                                                 error:&error];
            ...
            if (! jsonData) {
                ...
                NSLog(@"%@",str_JSON);
                NSDictionary *dict_UserINfo = [[NSDictionary alloc] initWithObjectsAndKeys:str_JSON,@"info", nil];
                [[NSNotificationCenter defaultCenter] postNotificationName:@"result" object:self userInfo:dict_UserINfo];
            }
            
            /*
             {"data":{"isButtonClickable":false,"isButtonInvisible":false,"isDeviceMode":false,"ppmValue":1,"time":0,"viewType":0},"key":"result"}
             */
        });
...
//
        //////Update view for Hold Your Breath Step
      
      //  [self updateCallBackForTimeViewUpdate:2];
        
         [self updateCallBackForTimeViewUpdate:2 withTime:15];
    }
    ...
    if(countdown ==5)
    {
       ...
       // [continueBtn setTitle:TranslatedString(@"Continue to Exhale", nil) forState:UIControlStateNormal];
        ...
        float offserReadingCount = [initialOffsetRecordings count];
        beforeRecordingCurrentFrequency = totalOffsets / offserReadingCount;
...
//
        ...
        ignoreReadings = NO;
        //[self updateCallBackForTimeViewUpdate:3];
         [self updateCallBackForTimeViewUpdate:3 withTime:0];
    }
    ...
    if(countdown >-6){
        if(countdown >0 && countdown <16)
        {
         ...
         //            {"data":{"buttonName":"","isButtonClickable":false,"isButtonInvisible":false,"isDeviceMode":true,"ppmValue":0,"time":2,"viewType":0},"key":"timeUpdate"}
            
            NSMutableDictionary* dictTimeUpdate = [[NSMutableDictionary alloc] init];
            [dictTimeUpdate setValue:@"timeUpdate" forKey:@"key"];
            NSMutableDictionary* dict_Buffer = [[NSMutableDictionary alloc] init];
             [dict_Buffer setValue:@"" forKey:@"buttonName"];
             [dict_Buffer setValue:@NO forKey:@"isButtonClickable"];
             [dict_Buffer setValue:@NO forKey:@"isButtonInvisible"];
             [dict_Buffer setValue:@YES forKey:@"isDeviceMode"];
             [dict_Buffer setValue:[NSNumber numberWithInt:0] forKey:@"ppmValue"];
             [dict_Buffer setValue:[NSNumber numberWithInt:countdown] forKey:@"time"];
             [dict_Buffer setValue:[NSNumber numberWithInt:0] forKey:@"viewType"];
             [dictTimeUpdate setValue:dict_Buffer forKey:@"data"];
            // [[NSNotificationCenter defaultCenter] postNotificationName:@"timeUpdate" object:self userInfo:dictTimeUpdate];
            ...
            if (! jsonData) {
                ...
                NSLog(@"%@",str_JSON);
                NSDictionary *dict_UserINfo = [[NSDictionary alloc] initWithObjectsAndKeys:str_JSON,@"info", nil];
                [[NSNotificationCenter defaultCenter] postNotificationName:@"timeUpdate" object:self userInfo:dict_UserINfo];
            }
        }else{
            if (countdown < 0) {
                // [self updateCallBackForTimeViewUpdate:3 withTime:countdown + 4];
                NSMutableDictionary* dictTimeUpdate = [[NSMutableDictionary alloc] init];
                [dictTimeUpdate setValue:@"timeUpdate" forKey:@"key"];
                NSMutableDictionary* dict_Buffer = [[NSMutableDictionary alloc] init];
                [dict_Buffer setValue:@"" forKey:@"buttonName"];
                ...
                [dict_Buffer setValue:@YES forKey:@"isDeviceMode"];
                [dict_Buffer setValue:[NSNumber numberWithInt:0] forKey:@"ppmValue"];
                [dict_Buffer setValue:[NSNumber numberWithInt:countdown + 4] forKey:@"time"];
                [dict_Buffer setValue:[NSNumber numberWithInt:3] forKey:@"viewType"];
                [dictTimeUpdate setValue:dict_Buffer forKey:@"data"];
                // [[NSNotificationCenter defaultCenter] postNotificationName:@"timeUpdate" object:self userInfo:dictTimeUpdate];
...
-(void)updateCallBackForTimeViewUpdate:(int) viewType withTime: (int)time{
    
    /*
     {"data":{"buttonName":"","isButtonClickable":false,"isButtonInvisible":true,"isDeviceMode":true,"ppmValue":0,"time":15,"viewType":2},"key":"viewTimeUpdate"}
     */
    NSMutableDictionary* dictTimeUpdate = [[NSMutableDictionary alloc] init];
    [dictTimeUpdate setValue:@"viewTimeUpdate" forKey:@"key"];
    NSMutableDictionary* dict_Buffer = [[NSMutableDictionary alloc] init];
    [dict_Buffer setValue:@"" forKey:@"buttonName"];
    [dict_Buffer setValue:@NO forKey:@"isButtonClickable"];
    [dict_Buffer setValue:@YES forKey:@"isButtonInvisible"];
    [dict_Buffer setValue:@YES forKey:@"isDeviceMode"];
    [dict_Buffer setValue:[NSNumber numberWithInt:0] forKey:@"ppmValue"];
    [dict_Buffer setValue:[NSNumber numberWithInt:time] forKey:@"time"];
    [dict_Buffer setValue:[NSNumber numberWithInt:viewType] forKey:@"viewType"];
    [dictTimeUpdate setValue:dict_Buffer forKey:@"data"];
    ...
    if (! jsonData) {
        ...
        NSLog(@"%@",str_JSON);
        NSDictionary *dict_UserINfo = [[NSDictionary alloc] initWithObjectsAndKeys:str_JSON,@"info", nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"viewTimeUpdate" object:self userInfo:dict_UserINfo];
    }
}
...
//        NSMutableDictionary *dict_Data_For_Notification = [[NSMutableDictionary alloc] init];
//        NSMutableDictionary *dict_Buffer_Data = [[NSMutableDictionary alloc] init];
//        [dict_Buffer_Data setValue:false forKey:@"isButtonClickable"];
//        [dict_Buffer_Data setValue:false forKey:@"isButtonInvisible"];
//        [dict_Buffer_Data setValue:false forKey:@"isDeviceMode"];
//        [dict_Buffer_Data setValue:[NSNumber numberWithInt: manualPPMReadingPrecise] forKey:@"ppmValue"];
//        [dict_Buffer_Data setValue:0 forKey:@"time"];
//        [dict_Buffer_Data setValue:0 forKey:@"viewType"];
//        [dict_Data_For_Notification setValue:@"result" forKey:@"key"];
//        [dict_Data_For_Notification setValue:dict_Buffer_Data forKey:@"data"];
//
//        [[NSNotificationCenter defaultCenter] postNotificationName:@"result" object:self userInfo:dict_Data_For_Notification];
    }
}
...
//        [dict_Buffer_Data setValue:false forKey:@"isButtonInvisible"];
//        [dict_Buffer_Data setValue:false forKey:@"isDeviceMode"];
//        [dict_Buffer_Data setValue:[NSNumber numberWithInt: ppm] forKey:@"ppmValue"];
//        [dict_Buffer_Data setValue:0 forKey:@"time"];
//        [dict_Buffer_Data setValue:0 forKey:@"viewType"];
//        [dict_Data_For_Notification setValue:@"result" forKey:@"key"];
//        [dict_Data_For_Notification setValue:dict_Buffer_Data forKey:@"data"];
...
-(IBAction)playFrequency
{
    ...
    if(triedPlayAndFailed)
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            // UI update code here.
            self.connectedDeviceAlert = [[UIAlertView alloc] initWithTitle:@"No Device Found"
                                                              message:@"An iCO device is not properly connected, please reconnect the device."
                                                             delegate:self
                                                    cancelButtonTitle:@"OK"
                                                    otherButtonTitles:nil];
           //  [self.connectedDeviceAlert addButtonWithTitle:@"Manual"];
            [self.connectedDeviceAlert show];
        });
    }
    else{
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if(alertView == connectedDeviceAlert)
    {
        if(buttonIndex ==0)
        {
            ...
            vertSlider.userInteractionEnabled = NO;
            //Send Notifcation for Device Mode = True
            
            /*
             {"data":{"isButtonClickable":false,"isButtonInvisible":false,"isDeviceMode":false,"ppmValue":0,"time":0,"viewType":0},"key":"deviceMode"}

             */
            
            [self updateCallBackWithDeviceMode:deviceMode];
        }
        else if(buttonIndex ==1)//manual
        {
            ...
            //Send  notification for device mode false
            [self updateCallBackWithDeviceMode:deviceMode];
        }
    }
}
...
//    if([audRoute rangeOfString:@"HeadsetInOut"].location != NSNotFound  )
    //  if([audRoute rangeOfString:@"Headphone"].location != NSNotFound  )
   if([audRoute rangeOfString:@"HeadsetInOut"].location != NSNotFound  )
    {
        audRoute = @"Connected";
        // [startListenBtn setEnabled:YES];
       // Hit the callback for Device Mode /////
    }
    else{
...
- (void) setupApplicationAudio {
    ...
    if ([[self audioRouteAvailable] isEqualToString: @"Connected"]) {
        //////Send CallBack Data to React NAtive //////
         [self updateCallBackWithDeviceMode:deviceMode];
...
//        {
        [self updateCallBackWithDeviceMode:YES];
            [self resetDeviceButtonTimer];
//        }
    }
    else{
        ...
        [self updateCallBackWithDeviceMode:NO];
        [self resetDeviceButtonTimer];
    }
    //[status release];
